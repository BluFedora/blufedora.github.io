...
com_blufedora_Animation.prototype = {
	...
	,setIndex: function(value) {
		...
		if(this._index >= this.steps.length) {
			return this.setIndex(this.props.loop ? 0 : -1);
		...
		while(_g < _g1.length) {
			...
			if(target == null) {
				...
				if(target.__properties__) {
					tmp = target.__properties__["get_" + field];
					value1 = tmp;
				} else {
					value1 = false;
				}
	...
	if(element != null) {
		...
		xobj.onreadystatechange = function() {
			if(xobj.readyState == 4 && xobj.status == 200) {
				...
				var _g1 = 0;
				var _g = data.length;
				while(_g1 < _g) creatFunc(element1,data[_g1++]);
			}
		};
		xobj.send(null);
	}
...
com_blufedora_portfolio_LoadPortfolio.load = function(path,element,creatFunc) {
	...
	xobj.onreadystatechange = function() {
		if(xobj.readyState == 4 && xobj.status == 200) {
			...
			var _g1 = 0;
			var _g = data.length;
			while(_g1 < _g) creatFunc(element,data[_g1++]);
		}
	};
	xobj.send(null);
};
...
com_blufedora_portfolio_Photos.setImage = function(i) {
	...
	var tmp = "url(" + (js_Boot.__cast(com_blufedora_portfolio_Photos.images[i] , HTMLDivElement)).dataset.image;
...
var com_blufedora_portfolio_Popup = function() {
	var _gthis = this;
	this.popUp = window.document.getElementById("pop-up");
	this.bg = window.document.getElementById("pop-up-bg");
	this.prev = window.document.getElementById("pop-up-prev");
	this.content = window.document.getElementById("pop-up-content");
	this.next = window.document.getElementById("pop-up-next");
