...
class Coll2D
{
    ...
    public static function testCircleVsPolygon(circle:Circle, polygon:Polygon, flip:Bool):Data
  {
        ...
        var normalAxis:Vec = new Vec();
        var offset : Float;
        var vectorOffset:Vec = new Vec();
        var vectors:Array<Vec>;
        var shortestDistance : Float = 0x3FFFFFFF;
        var collisionData:Data = new Data();
        var distMin : Float;

        var distance : Float = 0xFFFFFFFF;
        var testDistance : Float = 0x3FFFFFFF;
        var closestVector:Vec = new Vec(); //the vector to use to find the normal
        
        // find offset
        vectorOffset = new Vec(-circle.x,-circle.y);
        vectors = polygon.transformedVertices.copy();
        
        //adds some padding to make it more accurate
        if(vectors.length == 2) {
            var temp:Vec = new Vec(-(vectors[1].y - vectors[0].y), vectors[1].x - vectors[0].x);
        ...
        normalAxis = new Vec(closestVector.x - circle.x, closestVector.y - circle.y);
        ...
        for(i in 0 ... vectors.length) {
            ...
            normalAxis = Utils.findNormalAxis(vectors, i);
        ...
        collisionData.solid1 = if(flip) polygon else circle;
        collisionData.solid2 = if(flip) circle else polygon;
        collisionData.separation = new Vec(-collisionData.unitVector.x * collisionData.overlap,
    ...
    public static function testCircles(circle1:Circle, circle2:Circle):Data {
        ...
        if(distanceSquared < totalRadius * totalRadius) { //if your distance is less than the totalRadius square(because distance is squared)
            ...
            var collisionData:Data = new Data(); //new CollisionData class to hold all the data for this collision
            collisionData.solid1 = circle1;
            collisionData.solid2 = circle2;
            collisionData.unitVector = new Vec(circle1.x - circle2.x, circle1.y - circle2.y);
            collisionData.unitVector.normalize();
            collisionData.separation = new Vec(collisionData.unitVector.x * difference,
  ...
  public static function checkPolygons(polygon1:Polygon, polygon2:Polygon, flip:Bool):Data {
        ...
        var axis:Vec; //the normal axis for projection
        var offset : Float;
        var vectors1:Array<Vec>; //the points
        var vectors2:Array<Vec>; //the points
        var shortestDistance : Float = 0x3FFFFFFF;
        var collisionData:Data = new Data();
        ...
        if(vectors1.length == 2) {
            var temp:Vec = new Vec(-(vectors1[1].y - vectors1[0].y), vectors1[1].x - vectors1[0].x);
            temp.truncate(0.0000000001);
            vectors1.push(vectors1[1].add(temp));
        }
        if(vectors2.length == 2) {
            var temp:Vec = new Vec(-(vectors2[1].y - vectors2[0].y), vectors2[1].x - vectors2[0].x);
            temp.truncate(0.0000000001);
        ...
        for(i in 0 ... vectors1.length) {
            ...
            axis = Utils.findNormalAxis(vectors1, i);
        ...
        collisionData.solid1 = if(flip) polygon2 else polygon1;
        collisionData.solid2 = if(flip) polygon1 else polygon2;
        collisionData.separation = new Vec(-collisionData.unitVector.x * collisionData.overlap,
  ...
  /** Internal api - generate a bresenham line between given start and end points */
    @:noCompletion public static function bresenhamLine( start:Vec, end:Vec ) : Array<Vec> {
        ...
        for(x in Std.int(start.x) ... Std.int(end.x)) { //for each point
            ...
            if(steep) {
                points.push(new Vec(y, x)); //if its steep, push flipped version
            } else {
                points.push(new Vec(x, y)); //push normal
            }
