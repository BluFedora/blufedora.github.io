...
class Tweener
{
	...
	public static function init(window:Window):Bool
	{
		...
		Tweener.updateTweens(0);
		
		// trace("BluFedora's Tweener Library has Initialized");
	...
	private static function updateTweens(timestamp):Void
	{
		if (Tweener.start == null) Tweener.start = timestamp;
		
		var progress:Float = timestamp - start; // In Milliseconds
